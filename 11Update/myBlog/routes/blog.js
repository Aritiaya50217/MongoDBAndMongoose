...
router.get('/',function(req,res,next){
    ...
    Blogs.getAllBlogs(function(err,blogs){
        ...
        // แสดงหน้า index.ejs
        // {key:value}
        res.render("blogs/index",{data:"Test Website",blogs:blogs});
    });
...
router.get('/update/:id',function(req,res,next){
    Blogs.updateBlogs([req.params.id],function(err,blog){
        // ถ้า error แสดง error
        if(err) throw err
        // ถ้าไม่ error ให้ไปยัง editform.ejs 
        // blog จะเก็บค่า Object
...
router.post('/update',[
    ...
    if (!result.isEmpty()) {
        ...
        // เรียกใช้ function updatedBlog ที่อยู่ใน models/blog.js
        Blogs.updatedBlogs(data,function(err){
            if(err) console.log(err)
            // เมื่อ save จะไปยังหน้า /blogs
            res.redirect("/blogs")
        });
